...
// Root of THIS app (PULSE-TRAVEL-CALENDAR/)
...
const output = process.argv[2]
  || path.join(root, "..", "..", "releases", `PULSE-Calendar-v${VERSION}.html`);
...
// Also inline the small boot-loader style block that lives directly in <head>.
const inlineStyleBlocks = [...index.matchAll(/<style>([\s\S]*?)<\/style>/gi)]
  .map((m) => `<style>\n${m[1]}\n</style>`).join("\n");
...
const title = titleMatch ? titleMatch[1] : "PULSE Travel Calendar";
