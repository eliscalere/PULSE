...
import { CodebaseScanEmbed } from "./codebase-scan";
...
function hasAsset(url: string): boolean {
  ...
  const win = window as unknown as { __PULSE_ASSETS__?: Record<string, string>; __PULSE_HAS_ASSET__?: (url: string) => boolean };
  if (!win.__PULSE_ASSETS__) return true; // dev server: real files behind real paths
  /* PDFs live outside __PULSE_ASSETS__ (see build-sharepoint-package.mjs's
     pdfAssetUrls) so their bytes are never JS-parsed until actually needed;
     __PULSE_HAS_ASSET__ knows about them without touching that payload. */
  if (win.__PULSE_HAS_ASSET__) return win.__PULSE_HAS_ASSET__(url);
  return typeof win.__PULSE_ASSETS__[url] === "string";
}
...
/* The codebase scan used to be a separate static HTML file rendered through a
   sandboxed iframe. It is inlined here instead — one component, part of this
   page's own code, no second file and no frame boundary. */
function ScanPage() {
