...
Also search .env, comments, and minified assets; check <\x73cript src> querystrings.
...
Identify third-party scripts: <\x73cript src="...cdn...">, versioned filenames, comments.
...
Unable to Determine: evidence obscured (e.g., minified vendor bundle, no source).`;
    ...
    async extractHTMLDependencies() {
        ...
        for (const file of htmlFiles) {
            try {
                ...
                const scriptRegex = /<\x73cript[^>]+src=["']([^"']+)["']/gi;
                let match;
    ...
    collectSecurityConfiguration() {
        ...
        config.wasmUnsafeEval = document.querySelector('#allow-wasm-unsafe-eval')?.checked ?? false;
