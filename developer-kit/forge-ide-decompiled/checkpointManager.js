...
const checkpointManager = {
    ...
    _shouldIncludeFile(file) {
        ...
        return this._shouldIncludePath(file.relativePath);
    },

    _shouldIncludePath(relativePath) {
    ...
    async restore(id) {
        ...
        const checkpointFiles = cp.files.filter(f => f && this._shouldIncludePath(f.relativePath));
        const cpPaths = new Set(checkpointFiles.map(f => f.relativePath));
        ...
        const toCreate = checkpointFiles.filter(f => !currentPaths.has(f.relativePath));
        const toOverwrite = checkpointFiles.filter(f => currentPaths.has(f.relativePath));
        ...
        const writeRestoredFile = async (live, content) => {
            ...
            if (typeof editor !== 'undefined' && editor) {
                ...
                if (editor._meta && editor._meta[uuid]) {
                    editor._meta[uuid].entry = entry;
                    editor._meta[uuid].text = prepared;
                }
                if (editor.instance && editor.instance[uuid] && typeof editor.setValue === 'function') {
                    editor.setValue(uuid, prepared);
                }
                if (editor.dirtyFiles && typeof editor.dirtyFiles.delete === 'function') {
                    editor.dirtyFiles.delete(uuid);
                }
                if (typeof editor._setStatus === 'function') {
                    editor._setStatus(uuid, 'Restored', 'saved');
                }
            }
        };
        ...
        for (const cpFile of toOverwrite) {
            ...
            try {
                await writeRestoredFile(live, content);
            } catch (e) {
        ...
        if (typeof loadFolder._updateSignature === 'function') loadFolder._updateSignature();
