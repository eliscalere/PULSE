...
const editor = {
    ...
    _parseCompiledManifest(contents) {
        const match = String(contents || '').match(/<!--WFC-MANIFEST:([\s\S]*?)-->/);
        if (!match) return null;
        const decoded = this._decodeUtf8B64(String(match[1] || '').trim());
    ...
    _manifestLooksLikeForgeSelfHtml(manifest) {
        ...
        const paths = manifest.files
            ...
            .filter(Boolean);
        const hasPath = (name) => paths.some(path => path === name || path.endsWith('/' + name));
        const sourceModuleHits = [
            ...
            'compiler.js',
            'athenaagent.js'
        ].filter(hasPath).length;
        const projectName = String(manifest.project || '').trim().toLowerCase();
        return sourceModuleHits >= 4 || (projectName === 'forge' && sourceModuleHits >= 3);
    },

    _extractShippedChildHtml(contents) {
        const str = String(contents || '');
        if (!/CHILD_HTML_B64/.test(str)) return null;
        const match = str.match(/\b(?:const|let|var)\s+CHILD_HTML_B64\s*=\s*(["'])([A-Za-z0-9+/=]+)\1\s*;?/);
        if (!match) return null;
    ...
    _looksLikeForgeSelfHtml(contents) {
        ...
        const manifest = this._parseCompiledManifest(str);
        if (this._manifestLooksLikeForgeSelfHtml(manifest)) return true;

        if (/id=["']forge-secure-app-frame["']/i.test(str) && /CHILD_HTML_B64/i.test(str)) {
            const decodedChild = this._extractShippedChildHtml(str);
        ...
        const sourceModuleHits = [
            /<\x73cript[^>]+src=["']loadFolder\.js["']/i,
            /<\x73cript[^>]+src=["']editor\.js["']/i,
            /<\x73cript[^>]+src=["']checkpointManager\.js["']/i,
            /<\x73cript[^>]+src=["']compiler\.js["']/i,
            /<\x73cript[^>]+src=["']athenaAgent\.js["']/i
    ...
    ensureUnshippedBanner(contents, fileName = '', meta = null) {
        ...
        // <\x73cript> blocks (e.g. minified DOMPurify contains "<body>" as a JS string).
        const bodyRegex = /<body[^>]*>/gi;
        let bodyMatch;
        while ((bodyMatch = bodyRegex.exec(str)) !== null) {
            // Check whether this match sits inside a <\x73cript>…</script> block
            const before = str.slice(0, bodyMatch.index);
            const lastScriptOpen = before.lastIndexOf('<\x73cript');
            if (lastScriptOpen !== -1) {
                const lastScriptClose = before.lastIndexOf('</script');
                // If a <\x73cript was opened more recently than it was closed,
    ...
    _insertToolScriptsIntoHtml(contents, scriptEntries) {
        ...
        if (headEntries.length > 0) {
            const headTags = headEntries.map(item => `    <\x73cript src="${item.src}"></script>`);
            const headInsert = `\n${headTags.join('\n')}`;
        ...
        const tailTags = tailEntries.map(item => `    <\x73cript src="${item.src}"></script>`);
        const tailInsert = `\n${tailTags.join('\n')}\n`;
