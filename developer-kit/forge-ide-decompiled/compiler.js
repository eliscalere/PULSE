const compiler = {
    campfireShareUrl: 'https://flankspeed.sharepoint-mil.us/sites/Prometheus/SitePages/campfire.aspx',
...
1. **Use a pre-built Tailwind CSS CDN (recommended):**
   ...
   <\x73cript src="https://cdn.tailwindcss.com"></script>
...
Or uncheck "Add security headers" if you need the dynamic features.`
        ...
        {
            pattern: /<\x73cript[^>]*>[^<]*(?:eval|new\s+Function|document\.write)/i,
...
These APIs are blocked to prevent unexpected hardware access in offline/secure environments.`
    ...
    checkForSharePointDynamicInlineHandlerIssues(entryHtmlContent, jsFiles = []) {
        ...
        entryHtml.replace(/<\x73cript\b(?![^>]*\bsrc=)[^>]*>([\s\S]*?)</script>/gi, (_match, code) => {
            inlineScriptIndex += 1;
            sources.push({
    ...
    normalizeShipTarget(shipTarget) {
        ...
        if (raw === 'offline-ai' || raw === 'offline-api' || raw === 'offline-with-api' || raw === 'offline-with-apis') return 'offline-ai';
    ...
    getShipTargetLabel(shipTarget) {
        ...
        if (normalized === 'offline-ai') return 'Offline with AI APIs';
        return 'Offline';
    },
  ...
  <\x73cript>
    ...
    async startCompilation(options = {}) {
        ...
        const opts = {
            ...
            forceAllowAskSageApi: undefined,
            forceAllowGenAiMilApi: undefined,
            forceApiAllowlistRaw: undefined,
            forceSharePointCompatMode: undefined,
            forceAllowCdnPulldowns: undefined,
            forceCdnAllowlistRaw: undefined,
            ...options
        };
        ...
        const includeAdvancedDebug = (document.querySelector('#include-advanced-debug')?.checked ?? false);
	        ...
	        const apiAllowlistRaw = (opts.forceApiAllowlistRaw ?? document.querySelector('#security-connect-allowlist')?.value ?? '');
	        const sharePointCompatMode = (opts.forceSharePointCompatMode ?? document.querySelector('#sharepoint-compat-mode')?.checked ?? false);
	        const allowCdnPulldowns = (opts.forceAllowCdnPulldowns ?? document.querySelector('#allow-cdn-pulldowns')?.checked ?? false);
	        const cdnAllowlistRaw = (opts.forceCdnAllowlistRaw ?? document.querySelector('#security-cdn-allowlist')?.value ?? '');
	        const allowAskSageApi = (opts.forceAllowAskSageApi ?? document.querySelector('#allow-asksage-api')?.checked ?? false);
	        const allowGenAiMilApi = (opts.forceAllowGenAiMilApi ?? document.querySelector('#allow-genaimil-api')?.checked ?? false);
            const allowWasmUnsafeEval = (opts.allowWasmUnsafeEval ?? document.querySelector('#allow-wasm-unsafe-eval')?.checked ?? false);
        ...
        const buildInlineScriptTag = (code) => `<\x73cript>${sanitizeScriptContent(code)}</script>`;
        const buildInlinedScriptTag = (sourceTag, code) => {
            const attrs = (String(sourceTag || '').match(/<\x73cript\b([^>]*)>/i) || [])[1] || '';
            const typeAttr = (attrs.match(/\s+type\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/i) || [])[0] || '';
            return `<\x73cript${typeAttr}>\n${sanitizeScriptContent(code)}\n</sc` + `ript>`;
        };
        ...
        const collectInlineScriptValidationErrors = (htmlText, layerLabel) => {
            ...
            const scriptRe = /<\x73cript\b([^>]*)>([\s\S]*?)</script>/gi;
            let match;
            let scriptIndex = 0;
        ...
        // markup that shares the same line (for example: "<html><head>...<\x73cript src=...>").
        ...
        for (let i = 0; i < html.length; i++) {
            ...
            try {
                if (line && line.includes('<scr' + 'ipt') && line.includes('src=')) {
                    ...
                    const isAdvancedDebugTab = srcLower.includes('advanceddebugtab.js');
                    const isAdvancedDebug = srcLower.includes('advanceddebug.js') && !isAdvancedDebugTab;
                    const isLegacyTest = srcLower.includes('simpletest') || srcLower.includes('unittest.js') || srcLower.includes('unittest.plan.js');
                    if ((isDevconsole && !includeDevconsole) || (isTestRecorder && !includeTestRecorder) || (isAdvancedDebug && !includeAdvancedDebug) || isLegacyTest) {
                        html[i] = `<!-- ${line.trim()} -->`;
                        continue;
                    }
                }
        ...
        const buildCspContent = () => {
            ...
            if (allowWasmUnsafeEval || wasmAssets.length) {
        ...
        const extractHtmlTitle = (docHtml) => {
            const htmlText = String(docHtml || '')
                .replace(/<\x73cript\b[^>]*>[\s\S]*?</script>/gi, ' ')
        ...
        const escapeHtml = (text) => String(text || '')
            ...
            const childAllowAttr = 'clipboard-read *; clipboard-write *';
                ...
                '    const gestureCancelBtn = document.getElementById("forge-gesture-cancel");',
	                ...
	                '    const buildInlineScriptTag = (code) => "<\x73cript>" + String(code || "").replace(/<\\/script/gi, "<\\\\/script") + "</" + "script>";',
	                '    const MAILTO_ALLOWED = new Set(["subject","body","cc","bcc"]);',
