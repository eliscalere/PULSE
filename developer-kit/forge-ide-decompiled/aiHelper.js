...
<\x73cript src="https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js"></script>
...
- For the Prometheus SharePoint site, use \`https://flankspeed.sharepoint-mil.us/sites/Prometheus/\`. Do not use \`https://flankspeed.sharepoint-mil.us.mcas-gov.us/sites/Prometheus/\`.
...
- On startup or before the first SharePoint sync, run an \`ensureSharePointList()\` flow:
  ...
  4. Whether the list already existed or was just created, call \`ensureSharePointFields()\` to create any required custom fields that are missing.
  5. Re-fetch list metadata and cache \`ListItemEntityTypeFullName\`.
- Required fields are not optional setup text. Implement field creation in code, not only in the deployment notes.
- The minimum required custom fields are \`ItemType\` and \`DataJson\` unless the app already has a clearly better existing schema.
- Before any normal list item load/create/update, verify that \`ItemType\` and \`DataJson\` exist by checking list fields using \`InternalName\` or \`Title\`.
- If a required field is missing, create it through SharePoint REST, then verify it exists before continuing.
- Do not continue into normal reads/writes against a partially provisioned list. If field creation fails because of permissions, stop SharePoint sync and show a clear setup message with the exact list title, required columns, and column types.
...
- Be careful when merging \`fetch\` options. If \`options.headers\` is copied after the merged headers object, it can overwrite and remove the required \`Accept\` header.
- Safe pattern:
  - \`const headers = Object.assign({ Accept: 'application/json;odata=verbose' }, options.headers || {});\`
  - \`const fetchOptions = Object.assign({}, options, { credentials: 'include', headers });\`
- Unsafe pattern:
  - \`fetch(url, Object.assign({ credentials: 'include', headers }, options))\`
  - Reason: \`options.headers\` can overwrite the already-merged \`headers\` value, especially on POST calls that pass only \`Content-Type\`.
- If \`/_api/contextinfo\` returns XML such as \`<d:GetContextWebInformation>\` instead of JSON, first check whether the \`Accept: application/json;odata=verbose\` header was accidentally dropped by option merging.
...
----------------------
Use this for every startup/setup pass after confirming the list exists. Create only fields that are missing.
- Check fields:
  - \`/_api/web/lists/getbytitle('<APP_DATA_LIST_TITLE>')/fields?$select=Title,InternalName,TypeAsString&$filter=InternalName eq 'ItemType' or InternalName eq 'DataJson' or Title eq 'ItemType' or Title eq 'DataJson'\`
- Create \`ItemType\` if missing:
  - URL: \`/_api/web/lists/getbytitle('<APP_DATA_LIST_TITLE>')/fields/createfieldasxml\`
  - Body: \`{ parameters: { __metadata: { type: 'SP.XmlSchemaFieldCreationInformation' }, SchemaXml: "<Field Type='Text' DisplayName='ItemType' Name='ItemType' StaticName='ItemType' Required='FALSE' />", Options: 0 } }\`
- Create \`DataJson\` if missing:
  - URL: \`/_api/web/lists/getbytitle('<APP_DATA_LIST_TITLE>')/fields/createfieldasxml\`
  - Body: \`{ parameters: { __metadata: { type: 'SP.XmlSchemaFieldCreationInformation' }, SchemaXml: "<Field Type='Note' DisplayName='DataJson' Name='DataJson' StaticName='DataJson' Required='FALSE' RichText='FALSE' NumLines='20' />", Options: 0 } }\`
- Include \`X-RequestDigest\`, \`Content-Type: application/json;odata=verbose\`, and \`Accept: application/json;odata=verbose\` on field creation requests.
- After creating fields, re-query fields and fail with manual setup instructions if any required field is still missing.
...
- Implement idempotent list and field auto-provisioning: check for the configured list, create it if missing, verify required fields, create missing required fields, re-verify them, and only then continue with normal data load.
- Do not treat \`ItemType\` and \`DataJson\` as manual-only columns unless SharePoint permissions block field creation.
